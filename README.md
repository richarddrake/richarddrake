...
- 支持从网页接口文档提取请求方法、真实路径、请求参数和响应字段等接口事实，再由大模型扩展更多场景，并通过平台事实校验确保生成用例携带真实可执行 `api_test` 配置
- SSE 流式输出生成过程与测试用例
- 生成过程解析动态展示
- 测试用例卡片视图、表格视图、搜索过滤
...
- 支持单个接口用例集最多 500 个步骤，适合完整业务链路回归测试
- 支持接口用例集批量顺序执行、失败中断、最多 100 次请求 / 200 并发的基础性能冒烟和耗时指标
...
- 新增小牛电商订单测试实战项目需求分析、45 条接口测试用例和可执行用例集。
- 被测系统覆盖登录、鉴权、商品、购物车、订单、支付、取消和管理员重置。
- 接口用例集上限已从 30 步扩容到 500 步，前端执行用例集默认失败后继续执行。
- 实际执行 45 个接口步骤，40 个通过、5 个失败，步骤通过率 88.89%。
- 共执行 148 个断言，138 个通过、10 个失败，断言通过率 93.24%。
- 发现数量为 0 加购、库存为 0 加购、超库存加购、重复支付等业务缺陷。
